...
#set.seed(456)
simulated_df_acl <- map_dfc(1:n_repeats, ~ rnorm(length(values), mean = mean_value, sd = sd_val)) %>%
...
simulated_long_acl$Value <- ifelse(simulated_long_acl$Value > 120000, 120000, simulated_long_acl$Value)
...
# Simulate 100 samples per value
#set.seed(456)
simulated_df_acl_3yr <- map_dfc(1:n_repeats, ~ rnorm(length(values), mean = mean_value, sd = sd_val)) %>%
  set_names(paste0("Sample", 1:n_repeats))
...
simulated_long_acl_3yr$Value <- ifelse(simulated_long_acl_3yr$Value > 120000, 120000, simulated_long_acl_3yr$Value)
...
out <- data.frame(Annual_AM=Percent_over_1_yr_acl,
                  Three_Year_AM=Percent_over_3_yr_acl, PSE=pse )
